...
async function initMusicSystem() {
  try {
    ...
    window.musicLibrary = savedMusic || [];
...
function renderMusicLibrary() {
  ...
  list.innerHTML = window.musicLibrary.map((music, index) => `
    <div class="music-list-item ${window.currentMusicIndex === index ? 'playing' : ''}" onclick="selectMusic(${index})">
      <div class="music-list-index">${window.currentMusicIndex === index ? '▶' : (index + 1)}</div>
      <div class="music-list-info">
        <div class="music-list-name">${escapeHtml(music.name)}</div>
        <div class="music-list-artist">${escapeHtml(music.artist || '未知歌手')}</div>
      </div>
...
function openMusicImportModal() {
  window.importMusicData = { audioData: null, lrcText: '', name: '', artist: '' };
...
function checkImportValid() {
  ...
  document.getElementById('importMusicConfirmBtn').disabled = !(name && hasAudio);
}
...
function showMusicPlayer(music) {
  ...
  if (lyricsContainer) {
    if (window.parsedLyrics.length > 0) {
      lyricsContainer.innerHTML = window.parsedLyrics.map((item, i) => 
        `<div class="lyric-line" data-index="${i}">${escapeHtml(item.text)}</div>`
      ).join('');
    } else {
      lyricsContainer.innerHTML = '<div class="lyric-empty">暂无歌词，点击上方"编辑歌词"添加</div>';
    }
  }
}
...
function updateLyricHighlight(currentTime) {
  ...
  if (newIndex !== window.currentLyricIndex) {
    ...
    const lines = document.querySelectorAll('#musicLyricsContainer .lyric-line');
...
function updateAIMusicContext() {
  ...
  const currentLyric = window.currentLyricIndex >= 0 ? 
    window.parsedLyrics[window.currentLyricIndex]?.text : '';
  
  const contextRange = 2;
  const startIdx = Math.max(0, window.currentLyricIndex - contextRange);
  const endIdx = Math.min(window.parsedLyrics.length - 1, window.currentLyricIndex + contextRange);
  ...
  window.currentMusicContext = {
    ...
    currentTime: window.musicPlayer ? formatTime(window.musicPlayer.currentTime) : '0:00',
...
function updatePlayModeBtn() {
  ...
  const icons = {
    ...
    single: '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="17 1 21 5 17 9"></polyline><path d="M3 11V9a4 4 0 0 1 4-4h14"></path><polyline points="7 23 3 19 7 15"></polyline><path d="M21 13v2a4 4 0 0 1-4 4H3"></path><text x="12" y="14" font-size="8" fill="currentColor" text-anchor="middle">1</text></svg>',
    random: '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="16 3 21 3 21 8"></polyline><line x1="4" y1="20" x2="21" y2="3"></line><polyline points="21 16 21 21 16 21"></polyline><line x1="15" y1="15" x2="21" y2="21"></line><line x1="4" y1="4" x2="9" y2="9"></line></svg>'
  };
  
  btn.innerHTML = icons[window.playMode];
  btn.title = { list: '列表循环', single: '单曲循环', random: '随机播放' }[window.playMode];
}
...
function onMusicEnded() {
  ...
  if (window.playMode === 'single') {
    ...
    } while (nextIndex === window.currentMusicIndex && window.musicLibrary.length > 1);
    selectMusic(nextIndex);
  } else {
    // 列表循环
    const nextIndex = (window.currentMusicIndex + 1) % window.musicLibrary.length;
    selectMusic(nextIndex);
  }
}
...
function playPrevMusic() {
  ...
  if (window.playMode === 'random') {
    ...
    } while (prevIndex === window.currentMusicIndex && window.musicLibrary.length > 1);
  } else {
    prevIndex = (window.currentMusicIndex - 1 + window.musicLibrary.length) % window.musicLibrary.length;
  }
...
function seekMusicBy(seconds) {
  if (!window.musicPlayer) return;
  window.musicPlayer.currentTime = Math.max(0, Math.min(
...
function updateFloatingLyric() {
  ...
  if (lyricEl) {
    if (window.currentLyricIndex >= 0 && window.parsedLyrics[window.currentLyricIndex]) {
      lyricEl.textContent = window.parsedLyrics[window.currentLyricIndex].text;
    } else {
      lyricEl.textContent = window.currentMusic ? `♪ ${window.currentMusic.name} ♪` : '♪ ♪ ♪';
    }
  }
}

// 桌面歌词拖动
let lyricDragData = { isDragging: false, startX: 0, startY: 0, initialX: 0, initialY: 0 };

function initDesktopLyricDrag() {
  const lyric = document.getElementById('desktopLyric');
  if (!lyric) return;

  lyric.addEventListener('touchstart', handleLyricDragStart, { passive: false });
  lyric.addEventListener('mousedown', handleLyricDragStart);
  
  document.addEventListener('touchmove', handleLyricDragMove, { passive: false });
...
async function saveEditedLyrics() {
  ...
  const index = window.musicLibrary.findIndex(m => m.id === window.currentMusic.id);
  if (index >= 0) {
